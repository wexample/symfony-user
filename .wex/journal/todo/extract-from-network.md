...
- Steps 5, 6 and 7 done (2026-09-25), on scheb/2fa-bundle 8.6. Divergences from the plan:
  - Own provider (`Security\TwoFactor\EmailCodeTwoFactorProvider`, alias `email_code`) rather than scheb/2fa-email, which keeps the code in clear on the entity. `TwoFactorCodeService` keeps it in the session of the waiting login, hmac-hashed, 10 minutes, 5 attempts, resend after 60 s, and counts failures per account across sessions (20 per hour). The firewall login throttling counts the code attempts too.
  - Only a password login asks for the code: a magic link, or the login after a password reset, already proved the mailbox.
  - The code form is an ordinary ajax form (`TwoFactorCodeForm`, `otp-input` field); the firewall `check_path` points at its submission, `prepare_on_login: true` sends the code with the password check, `TwoFactorResponseHandler` answers JSON or redirects. The form page is `/login/2fa` (`TwoFactorController`), with resend and cancel (logout).
  - Trusted devices are scheb's signed cookie, not an entity: `AbstractUser::revokeTrustedDevices()` revokes them all, there is no per-device list or single revocation. The cookie is a JWT signed with `kernel.secret`, which must be 32 bytes at least.
...
- Tunnel steps done (2026-09-25): `LoginStep` and `AbstractUserMailStep`, ported from network's `Custom\LoginStep` and `Cart\UserMail` without the cart. `symfony-tunnels` is a plain requirement, not an optional one: the dependency check of wex only reads `require`. `symfony-tunnels` changed to let an account signing in take on the anonymous tunnel session of its own browser (never through a hash, never from one account to another). Demo: a sign-up tunnel on `/user/signup/user-mail`; resetting the demo accounts deletes the accounts it created.
- Mails translated (2026-09-25): every text, subject included, comes from `assets/mails/<type>.<locale>.yml`, read as `@mail::` — `symfony-translations` gained `Translator::DOMAIN_TYPE_MAIL`, set by the sender around the rendering. English and French. The locale is the one of the request; a mail sent from a worker uses the default one.
